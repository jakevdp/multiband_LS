...
Here we consider the SDSS II observations of 483 RR Lyrae stars compiled and studied by S10, in which periods for these stars were determined based on emperically-derived light curve templates.
...
One potentially fruitful avenue of research which we do not study here is the application of L1 regularization (also known as Lasso regression) to the higher-order periodogram.
This is similar in spirit to the Tikhonov regularization discussed in \sect{regularization}, but tends toward sparsity in the model parameters \citep[see, e.g.][for a discussion]{ICVG2014}.
...
Another potentially fruitful way forward in the multiband case would be to define and make use of physically-motivated priors in the light-curve shape.
...
Thus the template-fitting method, though extremely accurate, in practice requires several hours of CPU time for a well-sampled period grid for a single source (compared to several seconds for the multiband periodogram proposed here).
...
Rather than doing the full analysis including the final template fits, we will focus on the ability of the multiband periodogram to quickly provide suitable candidate periods under the assumption that the S10 template algorithm will then select or reject the optimial period from this set.
...
Because of our relaxed matching criteria, even the single-band supersmoother results here are much more optimistic than the \citet{Oluseyi12} results (compare to Figure 15 in that work): the supersmoother result here can be considered a best-case scenario for \adhoc{} single-band fits.
...
It also meets the second constraint: it requires the solution of a simple linear model at each frequency, compared to a sliding-window model in the case of supersmoother, a nonlinear optimization in the case of template-fitting, and a Markov Chain Monte Carlo in the case of CARMA models.
...
Despite pessimism in previous studies, our experiments with simulated LSST data lead us to expect such a hybrid approach to successfully identify periods in the vast majority of RR Lyrae stars brighter than $g\sim 22.5$ in the first months of the survey.
