...
\new{The underlying model of the Lomb-Scargle periodogram is nonlinear in frequency, and thus the likelihood surface in frequency is non-convex.
This non-convexity is readily apparent in the many local maxima of the periodogram, which makes it difficult to find the maximum via standard numerical optimization routines.
...
\old{The underlying model of the Lomb–Scargle periodogram is nonlinear in frequency and basis functions at different frequencies are not orthogonal. 
As a result, the periodogram has many local maxima. Thus in practice the global 
maximum of the periodogram is often found by a brute-force grid search (for details see, e.g. Ivezic 2014)}
...
Though this assumption is essential to the models presented in this section, it will become superfluous with the floating-mean models described in later sections.
...
\end{equation}
The periodogram derived from this model can be more accurate than the standard periodogram for certain observing cadences and selection functions. \citet{Zechmeister09} detail the required modifications to the orthogonalized harmonic formalism of \eq{LombScargle} to allow the mean to float in the model. In the matrix formalism, the modification is much more straightforward: all that is required is to add a column of ones to the $X_\omega$ matrix before computing the power via \eq{LombScargle2}. This column of ones corresponds to a third entry in the parameter vector $\theta$, and acts as a uniform constant offset for all data points.
...
\new{One might hope that when adding terms, the correct-period model would show more of an improvement than the incorrect-period model (and thus the maximum would become more pronounced), but this does not generally hold.
Consider that in the extreme limit in which the number model parameters is equal to the number of data points, the model has enough flexibility to fit the data perfectly at {\it every} frequency, and the resulting periodogram would be a flat line!
Thus when adding more terms, the boost in power to the background must be larger on average than the boost in power to the signal.}
...
\sectlabel{multiband}
In this section we will combine the ideas of the previous sections to construct the {\it multiband periodogram} which flexibly account for heterogeneous sources of data for a single object.
\new{To start with, we might consider one of two na{\"i}ve approaches:

First, we might consider ignoring band labels and simply compute a single standard Lomb-Scargle periodogram over the full dataset. This amounts to the assumption that a single sinusoid fits each band, and in practice will fail to account for astrophysical variability between bands.

Second, we might consider computing a standard Lomb-Scargle periodogram in each band, and using the additivity of $\chi^2$ along with \eq{chi2PN} to construct a multiband periodogram. This amounts to the assumption that the bands have completely independent phases and amplitudes, and has too many free parameters to be useful in most cases of interest (see \sect{relationship} for further discussion).}
...
\begin{enumerate}
  \item An $N_{base}$-term truncated Fourier fit which models \new{the shared variability among all $K$ observed bands}. \old{a latent parameter, which we'll call the ``overall variability''.}
  \item A set of $N_{band}$-term truncated Fourier fits, each of which models the residual within a single band from the \old{overall variability} \new{shared variability accounted for above}.
\end{enumerate}
...
\new{Note the potential for confusion: $N_{band}$ here is not the number of observed bands, but the number of Fourier components fit to the data each of the $K$ observed bands.}
...
\new{A well-known (though often unrecognized) deficiency of Lomb-Scargle-type periodograms on unevenly-sampled data is that they do not measure the power of the signal in question, but the power of the signal {\it convolved with the observing with the survey window function}.
For regularly-sampled timeseries, this convolution is the source of the aliasing related to the Nyquist sampling limit; for non-regular sampling, this aliasing generally happens to some degree at {\it all} frequencies!
Thus even if the signal has a single well-defined period, this aliasing generally causes multiple maxima in the measured periodogram.
...
This can be viewed as a result of the very model simplicity which causes its success in the case of sparse multiband data: it cannot disentangle bias in the model from bias due to the survey window.
