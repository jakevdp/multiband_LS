...
\begin{abstract}
  This paper introduces the {\it multiband periodogram} a general extension of the standard Lomb-Scargle approach to detecting periodic signals in time-domain data.
\end{abstract}
...
\begin{equation}
  ...
  P_N(\omega) = \frac{1}{2\sigma^2}\left[
    \frac{\left[\sum_k(y_k - \bar{d})\cos\omega(t_k - \tau)\right]^2}
    {\sum_k \cos^2\omega(t_k - \tau)}
    +
    \frac{\left[\sum_k(y_k - \bar{d})\sin\omega(t_k - \tau)\right]^2}
