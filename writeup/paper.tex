...
The fact that the multiband model does not select the top frequency each time points to its weaknesses: first, as a Fourier-like model, it tends to be tricked by a structured survey window function common in astronomical datasets.
This is a result of the very model simplicity which causes its success in the case of sparse multiband data: it cannot disentangle model bias from window bias.
...
Another potentially fruitful way forward in the multiband case would be to make use of physically-motivated priors in the light-curve shape.
...
One potential way forward is to move away from nonparametric models like supersmoother and lomb-scargle to template-fitting methods such as those used in S10.
Indeed, such methods perform well even for sparsely-sampled multiband data such as those from the PanSTARRS survey (B. Sesar, private communication); the primary drawback is that such blind template fits are computationally extremely expensive: they involve nonlinear optimizations over each of several hundred candidate templates at each of tens of thousands of candidate frequencies.
Thus the template-fitting method, though extremely accurate, in practice requires several hours of computing time for a well-sampled period grid for a single source, compared to several seconds for the multiband periodogram proposed here.
...
Rather than doing the full analysis including the final template fits, we will focus on the ability of the multiband periodogram to quickly provide suitable candidate periods, and here assume that the S10 template algorithm will then select or reject the optimial period from this set.
...
Because of our relaxed matching criteria, the dashed line here is everywhere higher than the \citet{Oluseyi12} results (compare to Figure 15 in that work); thus the supersmoother result here can be considered a best-case scenario for single-band fits.
Nevertheless, without fail the multiband result is superior to the single-band result. The improvement is most apparent for faint stars, where the greater model flexibility of the supersmoother causes it to over-fit the noisy data.

These results point to much more promising prospects for science with variable stars than previously reported.
...
Part of this improvement is due to the performance of the shared-phase multiband model with noisy data, and part of this improvement is due to the relaxed period-matching constraints enabled by the hybrid approach of non-parametric and template-based period determination.
...
This finding suggests that the multiband periodogram could have an important role to play in the analysis of variable stars in future surveys.
...
As we were finalizing this manuscript, we were made aware of a preprint of an independent exploration of a similar approach to multiband light curves \citep{Long2014}; we compare the two approaches in \app{long_comparison}.
