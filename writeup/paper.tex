...
(note that the template fitting method is too slow to apply to such a large dataset as expected from LSST; the compute time budget
to estimate periods for a billion stars on a 1000-core machine in a year is only 30 seconds per light curve). 

The results of the previous section show that even in the case of sparsely-sampled multiband data, the shared-phase periodogram 
finds the true period among its top candidates. Furthermore, even with such sparse data, the S10 template-fitting performs well, 
though computationally it is extremely expensive (hours of computing time for a well-sampled period grid with at least 10$^4$ points; B. Sesar, private communication) if a small set of candidate periods cannot be provided. This suggests the following procedure for determining periods in future multiband datasets:
...
To accomplish this, we use simulated LSST cadence \citep{opsim1, opsim2, opsim3} in 20 arbitrarily chosen fields
that are representative of the anticipated main survey temporal coverage.
We simulated a set of 50 RR Lyrae observations with the S10 templates, with a range of apparent magnitudes between 
$g=20$ and $g=24.5$, corresponding to bright-to-faint range of LSST main-survey observations. We compute expected 
photometric errors using eqs.~4--6  from \citet{Ivezic08LSST}. 
...
In particular, these results point to much more promising prospects for science with variable stars than previously reported.
...
As we were finalizing this manuscript, we were made aware of a preprint of an independent exploration of a similar approach to multiband light curves \citep{Long2014}; we compare the two approaches in \sect{long_comparison}.
